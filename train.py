"""
Treinamento de Visual Transformer (ViT) no CIFAR-100
----------------------------------------------------
Configurações:
  - patch_size = 6x6
  - num_heads = [4, 6]
  - num_encoder_blocks = [8, 12]
  - Aumento de dados incluído
...
PATCH_SIZE = 4       # Smaller patches work better for CIFAR-32
LEARNING_RATE = 3e-4 # Proven learning rate
SEED = 42

tf.random.set_seed(SEED)

# Mixed precision (acelera em GPU) - temporarily disabled for debugging
...
# =====================================================
# LOOP DE EXPERIMENTOS (FIXED CONFIGURATIONS)
# =====================================================
# Start with smaller, more reasonable configurations
heads_list = [4, 8]      # Reasonable head counts for our embedding dims
blocks_list = [4, 6]     # Fewer layers for faster training and stability
results = []

for heads in heads_list:
    for blocks in blocks_list:
        run_name = f"h{heads}_b{blocks}"
        print(f"\n==== Treinando {run_name} ====\n")
        ...
        model = create_vit(
            input_shape=(32, 32, 3),
            patch_size=PATCH_SIZE,  # Use the global PATCH_SIZE (now 4)
            num_classes=100,
        ...
        lr_schedule = keras.optimizers.schedules.CosineDecay(
            initial_learning_rate=LEARNING_RATE,  # Use global LEARNING_RATE
        ...
        # Callbacks: checkpoints + TensorBoard + early stopping + LR reduction
        callbacks = [
